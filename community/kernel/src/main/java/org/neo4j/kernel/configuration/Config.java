...
public class Config implements DiagnosticsProvider, Configuration
{
    ...
    public Set<String> identifiersFromGroup( Class<?> groupClass )
    {
        ...
        params.keySet().stream().map(pattern::matcher).forEach(matcher -> {
			if ( matcher.matches() )
            {
     ...
     */
    public void augment( Map<String,String> settings )
    {
     ...
     */
    public void augment( String setting, String value )
    {
     ...
     * GraphDatabaseSettings#strict_config_validation} is true.
     */
    public void augment( Config config )
    {
     ...
     * GraphDatabaseSettings#strict_config_validation} is true.
     */
    public void augmentDefaults( Setting<?> setting, String value )
    {
    ...
    public void updateDynamicSetting( String setting, String update, String origin )
    {
        ...
        synchronized ( params )
        {
            ...
            log.info( "Setting changed: '%s' changed from '%s' to '%s' via '%s'",
                    setting, oldValueIsDefault ? new StringBuilder().append("default (").append(oldValueForLog).append(")").toString() : oldValueForLog,
                    newValueIsDefault ? new StringBuilder().append("default (").append(newValueForLog).append(")").toString() : newValueForLog, origin );
    ...
    public <V> void registerDynamicUpdateListener( Setting<V> setting, BiConsumer<V,V> listener )
    {
        ...
        BiConsumer<String,String> projectedListener = ( oldValStr, newValStr ) ->
        {
            ...
            {
                log.error( new StringBuilder().append("Failure when notifying listeners after dynamic setting change; ").append("new setting might not have taken effect: ").append(e.getMessage()).toString(), e );
            }
        };
    ...
    public Map<String,ConfigValue> getConfigValues()
    {
        return configOptions.stream()
                ...
                {
                    throw new RuntimeException( new StringBuilder().append("Duplicate setting: ").append(val1.name()).append(": ").append(val1).append(" and ")
    ...
    public void dump( DiagnosticsPhase phase, Logger logger )
    {
		...
		params.entrySet().forEach(param -> logger.log("%s=%s", param.getKey(), obsfucateIfSecret(param)));
    }
    ...
    private static void loadFromFile( @Nonnull File file, @Nonnull Log log, boolean throwOnFileLoadFailure, Map<String,String> into )
    {
        if ( !file.exists() )
        {
            ...
            {
                throw new ConfigLoadIOException( new IOException( new StringBuilder().append("Config file [").append(file).append("] does not exist.").toString() ) );
            }
        ...
        catch ( IOException e )
        {
            if ( throwOnFileLoadFailure )
            {
                throw new ConfigLoadIOException( new StringBuilder().append("Unable to load config file [").append(file).append("].").toString(), e );
            }
    ...
    private Stream<BoltConnector> boltConnectors( @Nonnull Map<String,String> params )
    {
        return allConnectorIdentifiers( params ).stream().map( BoltConnector::new ).filter(
                c -> "bolt".equalsIgnoreCase( c.group.groupKey ) || BOLT == c.type.apply( params::get ) );
    }
    ...
    private Stream<HttpConnector> httpConnectors( @Nonnull Map<String,String> params )
    {
        return allConnectorIdentifiers( params ).stream()
                .map( Connector::new )
                .filter( c -> "http".equalsIgnoreCase( c.group.groupKey ) ||
                        "https".equalsIgnoreCase( c.group.groupKey ) ||
    ...
    public String toString()
    {
        return params.entrySet().stream()
                ...
                .map( entry -> new StringBuilder().append(entry.getKey()).append("=").append(obsfucateIfSecret( entry )).toString() )
