...
public class IndexSpecifier
{
    ...
    private static final String LABEL = new StringBuilder().append(":").append(WHITESPACE).append(or( identifier( GROUP_LABEL ), quotedIdentifier( GROUP_QUOTED_LABEL ) )).toString();
    ...
    private static final String FOLLOWING_PROPERTY = new StringBuilder().append(",").append(WHITESPACE).append(PROPERTY).toString();
    ...
    private static final Pattern PATTERN_PROPERTY_CLAUSE_END = Pattern.compile( new StringBuilder().append(CONTINUE).append(PROPERTY_CLAUSE_END).append(TERMINATOR).toString() ); // Terminating pattern.
	...
	private static IndexSpecifier parse( String specification, boolean allowIndexNameSpecs, boolean allowIndexPatternSpecs )
    {
        ...
        if ( !matcher.find() )
        {
            throw new IllegalArgumentException( new StringBuilder().append("Cannot parse index specification: '").append(specification).append("'").toString() );
        }
        ...
        if ( indexName != null )
        {
            ...
            {
                throw new IllegalArgumentException( new StringBuilder().append("Cannot parse index specification: '").append(specification).append("' - it looks like an index name, which is not allowed.").toString() );
            }
            ...
            throw new IllegalArgumentException( new StringBuilder().append("Invalid characters following index name: '").append(specification).append("'").toString() );
        }

        if ( !allowIndexPatternSpecs )
        {
            throw new IllegalArgumentException( new StringBuilder().append("Cannot parse index specification: '").append(specification).append("' - it looks like an index pattern, but an index name was expected.").toString() );
        }
        ...
        if ( !matcher.find() )
        {
            throw new IllegalArgumentException( new StringBuilder().append("Expected to find a property clause following the label: '").append(specification).append("'").toString() );
        }

        matcher.usePattern( PATTERN_FIRST_PROPERTY );
        if ( !matcher.find() )
        {
            throw new IllegalArgumentException( new StringBuilder().append("Expected to find a property in the property clause following the label: '").append(specification).append("'").toString() );
        }
        ...
        if ( !matcher.find() )
        {
            throw new IllegalArgumentException( new StringBuilder().append("The property clause is not terminated: '").append(specification).append("'").toString() );
        }
    ...
    {
        return group( new StringBuilder().append(group( first )).append("|").append(group( second )).toString() );
    }
    ...
    {
        return group( new StringBuilder().append("`").append(capture( name, oneOrMore( group( or( "[^`]", "``" ) ) ) )).append("`").toString() );
    }

	private static String group( String contents )
    {
        return new StringBuilder().append("(?:").append(contents).append(")").toString();
    }

	private static String capture( String name, String contents )
    {
        return new StringBuilder().append("(?<").append(name).append(">").append(contents).append(")").toString();
    }
