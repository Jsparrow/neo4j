...
abstract class TreeNode<KEY,VALUE>
{
	...
	static void setKeyCount( PageCursor cursor, int count )
    {
        if ( count < 0 )
        {
            throw new IllegalArgumentException( new StringBuilder().append("Invalid key count, ").append(count).append(". On tree node ").append(cursor.getCurrentPageId()).append(".").toString() );
        }
