...
public final class DateValue extends TemporalValue<LocalDate,DateValue>
{
     ...
     * </ul>
     */
    static final String DATE_PATTERN = new StringBuilder().append("(?:").append("(?<shortYear>[0-9]{4})(?:").append("(?<shortMonth>[0-9]{2})(?<shortDay>[0-9]{2})?|" // calendar date
).append("W(?<shortWeek>[0-9]{2})(?<shortDOW>[0-9])?|" // week date
).append(QUARTER_DATES ? "Q(?<shortQuarter>[0-9])(?<shortDOQ>[0-9]{2})?|" : "").append("(?<shortDOY>[0-9]{3}))")
			.append("|" // ordinal date
).append("(?<longYear>(?:[0-9]{4}|[+-][0-9]{1,9}))(?:").append("-(?<longMonth>[0-9]{1,2})(?:-(?<longDay>[0-9]{1,2}))?|" // calendar date
).append("-W(?<longWeek>[0-9]{1,2})(?:-(?<longDOW>[0-9]))?|" // week date
).append(QUARTER_DATES ? "-Q(?<longQuarter>[0-9])(?:-(?<longDOQ>[0-9]{1,2}))?|" : "").append("-(?<longDOY>[0-9]{3}))?").append(")")
			.toString(); // ordinal date
