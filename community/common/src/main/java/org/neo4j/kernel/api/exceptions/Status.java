    ...
    */
	...
	enum Transaction implements Status
    {
        ...
        ForbiddenDueToTransactionType( ClientError,
                new StringBuilder().append("The transaction is of the wrong type to service the request. For instance, a transaction that has ").append("had schema modifications performed in it cannot be used to subsequently perform data operations, ").append("and vice versa.").toString() ),
        ...
        InvalidBookmark( ClientError,
                new StringBuilder().append("Supplied bookmark cannot be interpreted. You should only supply a bookmark previously that was ").append("previously generated by Neo4j. Maybe you have generated your own bookmark, ").append("or modified a bookmark since it was generated by Neo4j.").toString() ),
        ...
        DeadlockDetected( TransientError,
                new StringBuilder().append("This transaction, and at least one more transaction, has acquired locks in a way that it will wait ").append("indefinitely, and the database has aborted it. Retrying this transaction will most likely be ").append("successful.").toString() ),
        InstanceStateChanged( TransientError,
                new StringBuilder().append("Transactions rely on assumptions around the state of the Neo4j instance they ").append("execute on. For instance, transactions in a cluster may expect that ").append("they are executing on an instance that can perform writes. However, ").append("instances may change state while the transaction is running. This causes ").append("assumptions the instance has made about how to execute the transaction ").append("to be violated - meaning the transaction must be rolled ").append("back. If you see this error, you should retry your operation in a new transaction.")
						.toString() ),
        ...
        LockClientStopped( TransientError,
                new StringBuilder().append("The transaction has been terminated, so no more locks can be acquired. This can occur because the ").append("transaction ran longer than the configured transaction timeout, or because a human operator manually ").append("terminated the transaction, or because the database is shutting down.").toString() ),
	...
	enum Statement implements Status
    {
        ...
        ExhaustiveShortestPathWarning( ClientNotification,
                new StringBuilder().append("Exhaustive shortest path has been planned for your query that means that shortest path graph ").append("algorithm might not be used to find the shortest path. Hence an exhaustive enumeration of all paths ").append("might be used in order to find the requested shortest path.").toString() ),
	...
	enum Procedure implements Status
    {
        ...
        ProcedureNotFound( ClientError,
                new StringBuilder().append("A request referred to a procedure that is not registered with this database instance. If you are ").append("deploying custom procedures in a cluster setup, ensure all instances in the cluster have the ").append("procedure jar file deployed.").toString() ),
	...
	enum General implements Status
    {
        ...
        OutOfMemoryError( TransientError,
                new StringBuilder().append("There is not enough memory to perform the current task. Please try increasing ").append("'dbms.memory.heap.max_size' in the neo4j configuration (normally in 'conf/neo4j.conf' or, if you ").append("you are using Neo4j Desktop, found through the user interface) or if you are running an embedded ").append("installation increase the heap by using '-Xmx' command line flag, and then restart the database.").toString() ),
        StackOverFlowError( TransientError,
                new StringBuilder().append("There is not enough stack size to perform the current task. This is generally considered to be a ").append("database error, so please contact Neo4j support. You could try increasing the stack size: ").append("for example to set the stack size to 2M, add `dbms.jvm.additional=-Xss2M' to ").append("in the neo4j configuration (normally in 'conf/neo4j.conf' or, if you are using ").append("Neo4j Desktop, found through the user interface) or if you are running an embedded installation ").append("just add -Xss2M as command line flag.").toString() ),
	...
	class Code
    {
        ...
        {
            return new StringBuilder().append("Status.Code[").append(serialize()).append("]").toString();
        }
