...
public class DatabaseActions
{
	...
	private final Function<ConstraintDefinition,Representation> constraintDefToRepresentation =
            ...
            BadInputException
    {
        ...
        {
            Node node = node( nodeId );
            labelNames.forEach(labelName -> node.addLabel(label(labelName)));
        }
...

    {
        ...
        try
        {
            ...
            // Add new labels
			labels.forEach(labelName -> node.addLabel(label(labelName)));
        }
        catch ( org.neo4j.graphdb.ConstraintViolationException e )
        {
            throw new BadInputException( "Unable to add label, see nested exception.", e );
	...
	public void removeNodeIndex( String indexName )
    {
        ...
        {
            throw new NotFoundException( new StringBuilder().append("No node index named '").append(indexName).append("'.").toString() );
        }
	...
	public void removeRelationshipIndex( String indexName )
    {
        ...
        {
            throw new NotFoundException( new StringBuilder().append("No relationship index named '").append(indexName).append("'.").toString() );
        }
	...
	public ListRepresentation getNodePropertyExistenceConstraint( String labelName, Iterable<String> propertyKeys )
    {
        ...
        {
            Iterable<Representation> representationIterable = map( constraintDefToRepresentation, constraints );
	...
	public ListRepresentation getConstraints()
    {
        return new ListRepresentation( CONSTRAINT_DEFINITION,
                map( constraintDefToRepresentation, graphDb.schema().getConstraints() ) );
    }

	public ListRepresentation getLabelConstraints( String labelName )
    {
        return new ListRepresentation( CONSTRAINT_DEFINITION, map( constraintDefToRepresentation,
    ...
    private class UniqueRelationshipFactory extends UniqueFactory.UniqueRelationshipFactory
    {
        ...
        {
            (properties == null ? indexed : properties).entrySet().forEach(property -> relationship.setProperty(property.getKey(), property.getValue()));
        }
    }
    ...
    private class UniqueNodeFactory extends UniqueFactory.UniqueNodeFactory
    {
        ...
        {
            (properties == null ? indexed : properties).entrySet().forEach(property -> node.setProperty(property.getKey(), property.getValue()));
        }
    }
